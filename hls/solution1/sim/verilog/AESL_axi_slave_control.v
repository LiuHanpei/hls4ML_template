...
//------------------------Parameter----------------------
`define TV_IN_data_out "../tv/cdatafile/c.mem_streaming.autotvin_data_out.dat"
`define TV_IN_data_in "../tv/cdatafile/c.mem_streaming.autotvin_data_in.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter data_out_DEPTH = 1;
reg [31 : 0] data_out_OPERATE_DEPTH = 0;
parameter data_out_c_bitwidth = 64;
parameter data_in_DEPTH = 1;
reg [31 : 0] data_in_OPERATE_DEPTH = 0;
parameter data_in_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter mem_streaming_continue_addr = 0;
parameter mem_streaming_auto_start_addr = 0;
parameter data_out_data_in_addr = 16;
parameter data_in_data_in_addr = 28;
...
reg  BREADY_reg = 0;
reg [data_out_c_bitwidth - 1 : 0] mem_data_out [data_out_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_data_out [ (data_out_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * data_out_DEPTH -1 : 0] = '{default : 'hz};
reg data_out_write_data_finish;
reg [data_in_c_bitwidth - 1 : 0] mem_data_in [data_in_DEPTH - 1 : 0] = '{default : 'h0};
...
//write data_out reg
reg [31 : 0] write_data_out_count = 0;
reg [31 : 0] data_out_diff_count = 0;
reg write_data_out_run_flag = 0;
reg write_one_data_out_data_done = 0;
//write data_in reg
reg [31 : 0] write_data_in_count = 0;
reg [31 : 0] data_in_diff_count = 0;
...
assign TRAN_control_write_data_finish = 1 & data_out_write_data_finish & data_in_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_data_out_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (data_out_c_bitwidth, data_out_DEPTH, data_out_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_data_out_run_flag <= 1; 
        end
        else if ((write_one_data_out_data_done == 1 && write_data_out_count == data_out_diff_count - 1) || data_out_diff_count == 0) begin
            write_data_out_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_data_out_count = 0;
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_data_out_count = 0;
        end
        if (write_one_data_out_data_done === 1) begin
            write_data_out_count = write_data_out_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        data_out_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            data_out_write_data_finish <= 0;
        end
        if (write_data_out_run_flag == 1 && write_data_out_count == data_out_diff_count) begin
            data_out_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_data_out
    ...
    integer j;
    integer k;
    reg [31 : 0] data_out_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    ...
    while (1) begin
        ...
        data_out_diff_count = 0;

        for (k = 0; k < data_out_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (data_out_c_bitwidth < 32) begin
                    data_out_data_tmp_reg = mem_data_out[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < data_out_c_bitwidth) begin
                            data_out_data_tmp_reg[j] = mem_data_out[k][i*32 + j];
                        end
                        else begin
                            data_out_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_data_out[k * four_byte_num  + i]!==data_out_data_tmp_reg) begin
                data_out_diff_count = data_out_diff_count + 1;
                end
            end
        end
...
initial begin : write_data_out
    integer write_data_out_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_data_out_count = 0; check_data_out_count < data_out_OPERATE_DEPTH; check_data_out_count = check_data_out_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_data_out_run_flag === 1 && get_ack === 1) begin
                    process_busy = 1;
                    //write data_out data 
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (data_out_c_bitwidth < 32) begin
                            data_out_data_tmp_reg = mem_data_out[check_data_out_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < data_out_c_bitwidth) begin
                                    data_out_data_tmp_reg[j] = mem_data_out[check_data_out_count][i*32 + j];
                                end
                                else begin
                                    data_out_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_data_out[check_data_out_count * four_byte_num  + i]!==data_out_data_tmp_reg) begin
                        write (data_out_data_in_addr + check_data_out_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, data_out_data_tmp_reg, write_data_out_resp);
                        write_one_data_out_data_done <= 1;
                        @(posedge clk);
                        write_one_data_out_data_done <= 0;
                        image_mem_data_out[check_data_out_count * four_byte_num + i]=data_out_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        else if ((write_one_data_in_data_done == 1 && write_data_in_count == data_in_diff_count - 1) || data_in_diff_count == 0) begin
            write_data_in_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_data_in_run_flag == 1 && write_data_in_count == data_in_diff_count) begin
            data_in_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_data_in
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    integer c_bitwidth;
    integer i;
    ...
    while (1) begin
        wait (AESL_ready_reg === 1);
        data_in_diff_count = 0;

        for (k = 0; k < data_in_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (data_in_c_bitwidth < 32) begin
                    data_in_data_tmp_reg = mem_data_in[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < data_in_c_bitwidth) begin
                            data_in_data_tmp_reg[j] = mem_data_in[k][i*32 + j];
                        end
                        else begin
                ...
                if(image_mem_data_in[k * four_byte_num  + i]!==data_in_data_tmp_reg) begin
                data_in_diff_count = data_in_diff_count + 1;
                end
            end
        end
...
initial begin : write_data_in
    ...
    while (1) begin
        ...
        for (check_data_in_count = 0; check_data_in_count < data_in_OPERATE_DEPTH; check_data_in_count = check_data_in_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_data_in_run_flag === 1 && get_ack === 1) begin
                    ...
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (data_in_c_bitwidth < 32) begin
                            data_in_data_tmp_reg = mem_data_in[check_data_in_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < data_in_c_bitwidth) begin
                                    data_in_data_tmp_reg[j] = mem_data_in[check_data_in_count][i*32 + j];
                                end
                                else begin
                                    data_in_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_data_in[check_data_in_count * four_byte_num  + i]!==data_in_data_tmp_reg) begin
                        write (data_in_data_in_addr + check_data_in_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, data_in_data_tmp_reg, write_data_in_resp);
                        write_one_data_in_data_done <= 1;
                        @(posedge clk);
                        write_one_data_in_data_done <= 0;
                        image_mem_data_in[check_data_in_count * four_byte_num + i]=data_in_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
// Read data from file 
initial begin : read_data_out_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [data_out_c_bitwidth - 1 : 0] token_tmp; 
  ...
  fp = $fopen(`TV_IN_data_out ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_data_out); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < data_out_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_data_out [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
