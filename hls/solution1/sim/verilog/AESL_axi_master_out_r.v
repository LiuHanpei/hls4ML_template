...
//------------------------Parameter----------------------
`define TV_IN_out_r "../tv/cdatafile/c.mem_streaming.autotvin_out_r.dat"
`define TV_OUT_out_r "../tv/rtldatafile/rtl.mem_streaming.autotvout_out_r.dat"
 parameter out_r_ADDR_BITWIDTH = 32'd 64;
 parameter out_r_AWUSER_BITWIDTH = 32'd 1;
 parameter out_r_DATA_BITWIDTH = 32'd 256;
...
parameter out_r_C_DATA_BITWIDTH = 32'd 256;
parameter out_r_transaction_depth = 32'd 5;
parameter out_r_mem_depth = 32'd 5;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > out_r_mem_depth) begin
                $display ("F:/vivado_projects/hls_4ML/axi_port_fixed_point/solution1/sim/verilog/AESL_axi_master_out_r.v: Write request address %d exceed AXI master out_r array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, out_r_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > out_r_mem_depth) begin
                $display ("F:/vivado_projects/hls_4ML/axi_port_fixed_point/solution1/sim/verilog/AESL_axi_master_out_r.v: Read request address %d exceed AXI master out_r array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, out_r_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < out_r_transaction_depth; i = i + 1) begin 
          ...
          if (i%1 == 0) begin
              case(mem_page)
                  0 : out_r_mem_0[i/1] = mem_tmp;
                  1 : out_r_mem_1[i/1] = mem_tmp;
                  2 : out_r_mem_2[i/1] = mem_tmp;
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      mem_page = transaction_num % mem_page_num ;
      # 0.1;
      fp = $fopen(`TV_OUT_out_r, "ab"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_out_r); 
          $finish; 
      end 
      bin_data = out_r_transaction_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < out_r_mem_depth; i = i + 1) begin 
          case(mem_page)
              0 : tmp_data = out_r_mem_0[i];
              1 : tmp_data = out_r_mem_1[i];
              2 : tmp_data = out_r_mem_2[i];
